...
//! Forward repeated finalize storage reads until a conservative effect boundary.
...
impl Pass for StorageReadForwarding {
    ...
    fn do_pass(_input: Self::Input, state: &mut CompilerState) -> Result<Self::Output> {
        ...
        let mut visitor =
            StorageReadForwardingVisitor { state, reads: Default::default(), aliases: Default::default() };
